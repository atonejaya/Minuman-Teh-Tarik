...
- Produces: RPC `public.admin_reset_data(p_confirm text) returns jsonb` â€” dipanggil Task 2 via `supabase.rpc('admin_reset_data', { p_confirm: confirm })`. Return shape: `{ success: boolean, message: string }`. Error â†’ `raise exception`.
...
- Tidak ada `CREATE TABLE` â€” hanya fungsi.
