...
class CollectionController {
  ...
  async addInvoice(req, res) {
    try {
      ...
      const item = await collectionService.addInvoice(parseInt(id), parseInt(transaction_id), req.user.id);
  ...
  async finish(req, res) {
    try {
      const { id } = req.params;
      const collection = await collectionService.finishCollection(parseInt(id), req.body, req.user.id);
