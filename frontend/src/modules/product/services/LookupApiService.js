  ...
  async getLookups() {
    try {
      const queries = await Promise.all([
        ...
        supabase.from('Product').select('id, code, name, sku, category_id, brand_id, unit_id, cost_price, selling_price, is_active').eq('is_active', true).order('name'),
      ...
      const [warehouses, salesmen, units, products, categories, priceLevels, areas, routes, brands, taxes, packagings] = queries;
