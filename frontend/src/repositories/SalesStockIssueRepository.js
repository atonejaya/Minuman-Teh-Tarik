...
const SalesStockIssueRepository = {
  async getSalesStockIssues(params) {
    let query = supabase
      ...
      .select('*, sales:User(name), warehouse:Warehouse(name, code)', { count: 'exact' });
  ...
  async getSalesStockIssue(id) {
    const { data, error } = await supabase
      .from('SalesStockIssue')
      .select('*, items:SalesStockIssueItem(*, product:Product(*)), sales:User(name), warehouse:Warehouse(name, code), history:SalesStockIssueHistory(*)')
      .eq('id', id)
      .single();
    if (error) throw error;
    return { data };
